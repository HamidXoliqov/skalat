@extends('layouts.default')
    ...
    @section('meta')
        <title>Reports price | Workday Time Clock</title>
    ...
    @section('content')
    @include('admin.modals.modal-add-report-price')

    <div class="container-fluid">
        <div class="row">
            <h2 class="page-title">{{ __('Reports people') }}
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @php 
                                        if($ss->time_format == 1) {
                                            echo e(date('h:i:s A', strtotime($d->statr_date)));
                                        } else {
                                            echo e(date('H:i:s', strtotime($d->statr_date)));
                                        }
                                    @endphp
                                </td>
                                <td>
                                    @isset($d->end_date)
                                        @php 
                                            if($ss->time_format == 1) {
                                                echo e(date('h:i:s A', strtotime($d->end_date)));
                                            } else {
                                                echo e(date('H:i:s', strtotime($d->end_date)));
                                            }
                                        @endphp
                                    @endisset
                                </td>
                                ...
                                <td class="align-right">
                                    <a href="{{route('report-price.edit',$d->id)}}" class="ui circular basic icon button tiny"><i class="edit outline icon"></i></a>
                                    <div style="display: inline-table;">
                                    {!! Form::open(['route' => ['report-price.destroy',$d->id],'method'=>'DELETE']) !!}
                                        <button  class="ui circular basic icon button tiny"><i class="trash alternate outline icon"></i></button>
                                    {!! Form::close() !!}
                                    </div>
                                </td>
                            </tr>
